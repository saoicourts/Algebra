...
\end{rmk}
\begin{defn}
	A linear algebraic group $G\to\Spec k$ is called \textbf{linear reductive} if every representation decomposes as a direc sum of irreducibles.
\end{defn}
\begin{prob}
...
\end{pmatrix}\right\}
\subset\GL_2(k)\]
where $\Ga$ acts on $k^2$ by $\alpha(x,y)=(a+\alpha y,y)$. Then it can be easily seen not to be alinear representation.
